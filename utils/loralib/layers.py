...
class TimmAttentionLoRA(nn.Module):
    def __init__(
        self,
        existing_attn: "timm.models.vision_transformer.Attention",
        ...
        **kwargs
        ...
        super().__init__()
        # Extract key parameters from the existing attention layer
        self.num_heads = existing_attn.num_heads
        self.scale = existing_attn.scale  # scaling factor for dot-product (usually head_dim**-0.5)
        embed_dim = existing_attn.qkv.in_features  # input dimension (token embedding size)
        out_dim = embed_dim  # output dimension is same as embed_dim for self-attention
        
        # Initialize new Q, K, V linear layers and the output projection
        bias_flag = existing_attn.qkv.bias is not None
        self.q_proj = nn.Linear(embed_dim, embed_dim, bias=bias_flag)
        self.k_proj = nn.Linear(embed_dim, embed_dim, bias=bias_flag)
        self.v_proj = nn.Linear(embed_dim, embed_dim, bias=bias_flag)
        self.proj  = nn.Linear(out_dim, out_dim, bias=(existing_attn.proj.bias is not None))
        
        # Copy weights and biases from existing_attn into the new layers
        with torch.no_grad():
            # Original qkv weight is shape [3*embed_dim, embed_dim]
            qkv_weight = existing_attn.qkv.weight.data
            qkv_bias = existing_attn.qkv.bias.data if bias_flag else None
            # Copy Q weights/bias (indices [0:embed_dim])
            self.q_proj.weight.data.copy_(qkv_weight[0:embed_dim, :])
            if bias_flag:
                self.q_proj.bias.data.copy_(qkv_bias[0:embed_dim])
            # Copy K weights/bias (indices [embed_dim:2*embed_dim])
            self.k_proj.weight.data.copy_(qkv_weight[embed_dim:2*embed_dim, :])
            if bias_flag:
                self.k_proj.bias.data.copy_(qkv_bias[embed_dim:2*embed_dim])
            # Copy V weights/bias (indices [2*embed_dim:3*embed_dim])
            self.v_proj.weight.data.copy_(qkv_weight[2*embed_dim:3*embed_dim, :])
            if bias_flag:
                self.v_proj.bias.data.copy_(qkv_bias[2*embed_dim:3*embed_dim])
            # Copy output projection weight/bias
            self.proj.weight.data.copy_(existing_attn.proj.weight.data)
            if self.proj.bias is not None:
                self.proj.bias.data.copy_(existing_attn.proj.bias.data)
        
        # Preserve the dropout layers for attention and output (use same probabilities as original)
        self.attn_drop = nn.Dropout(existing_attn.attn_drop.p) if hasattr(existing_attn, 'attn_drop') else nn.Identity()
        self.proj_drop = nn.Dropout(existing_attn.proj_drop.p) if hasattr(existing_attn, 'proj_drop') else nn.Identity()
        
        # Initialize LoRA parameters (if LoRALayer base class is available)
        # This typically sets up attributes for LoRA scaling and dropout
        ...
        if 'q' in enable_lora:
            self.q_proj = LinearLoRA(
                self.q_proj, r=r, lora_alpha=lora_alpha, 
                fan_in_fan_out=False, dropout_rate=dropout_rate
            )
        if 'k' in enable_lora:
            self.k_proj = LinearLoRA(
                self.k_proj, r=r, lora_alpha=lora_alpha, 
                fan_in_fan_out=False, dropout_rate=dropout_rate
            )
        if 'v' in enable_lora:
            self.v_proj = LinearLoRA(
                self.v_proj, r=r, lora_alpha=lora_alpha, 
                fan_in_fan_out=False, dropout_rate=dropout_rate
            )
        if 'o' in enable_lora:
            self.proj = LinearLoRA(
                self.proj, r=r, lora_alpha=lora_alpha, 
                fan_in_fan_out=False, dropout_rate=dropout_rate
            )
    
    def forward(self, x: torch.Tensor):
        # x is expected to be of shape [B, N, C] (batch, sequence length, embed_dim)
        B, N, C = x.shape
        head_dim = C // self.num_heads
        
        # Compute Q, K, V projections (match dtype to input for safety in mixed precision)
        q = self.q_proj.to(dtype=x.dtype)(x)  # shape [B, N, C]
        k = self.k_proj.to(dtype=x.dtype)(x)  # shape [B, N, C]
        v = self.v_proj.to(dtype=x.dtype)(x)  # shape [B, N, C]
        
        # Reshape into [B, heads, N, head_dim] for attention computation
        q = q.reshape(B, N, self.num_heads, head_dim).permute(0, 2, 1, 3)  # [B, heads, N, head_dim]
        k = k.reshape(B, N, self.num_heads, head_dim).permute(0, 2, 1, 3)  # [B, heads, N, head_dim]
        v = v.reshape(B, N, self.num_heads, head_dim).permute(0, 2, 1, 3)  # [B, heads, N, head_dim]
        ...
        x_out = self.proj.to(dtype=x_out.dtype)(x_out)              # [B, N, C]
        x_out = self.proj_drop(x_out)
        
        # return x_out, attn  # Return the output features and attention matrix
        return x_out   # ✅ match timm’s Attention
    
    def train(self, mode: bool = True):
        super().train(mode)
        # If the LoRALayer base class defines a special training mode handling (e.g., freezing base weights),
        # it can be invoked here. For example:
        # self.lora_train(mode)
        return self
...
class PlainMultiheadAttentionLoRA(nn.Module):
    ...
    def forward_module(
        ...
        q = self.q_proj.to(dtype=query.dtype)(query)
        k = self.k_proj.to(dtype=key.dtype)(key)
        v = self.v_proj.to(dtype=value.dtype)(value)
        ...
        attn_output = self.proj.to(dtype=attn_output.dtype)(attn_output)
