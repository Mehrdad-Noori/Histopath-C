...
from .layers import LoRALayer, PlainMultiheadAttentionLoRA, TimmAttentionLoRA
from utils.misc import TorchMHAWrapper
...
def apply_lora(lora_dict, clip_model):
    if lora_dict['model'][-2:] == '32':
        lora_dict['model'] = 'ViT-B/32'
    elif lora_dict['model'][-2:] == '16':
        lora_dict['model'] = 'ViT-B/16'
    elif lora_dict['model'][-2:] == '14':
        lora_dict['model'] = 'ViT-L/14'
    list_lora_layers = []

    target_layers = lora_dict['layers']
    print(f"+++ Applying LoRA to layers {target_layers}")

    if lora_dict['encoder'] == 'text' or lora_dict['encoder'] == 'both':
        indices = INDEX_POSITIONS_TEXT[lora_dict['position']]
        text_encoder = clip_model.transformer
        for i, block in enumerate(text_encoder.resblocks):
            if i in indices and i in target_layers:
                print(f"Applying Lora on Attention Block {i}")
                for name, submodule in block.named_children():
                    if isinstance(submodule, nn.MultiheadAttention):
                        new_multi_head_lora = PlainMultiheadAttentionLoRA(
                            submodule, enable_lora=lora_dict['params'], r=lora_dict['r'], lora_alpha=lora_dict['alpha'],
                            dropout_rate=lora_dict['dropout_rate'])
                        setattr(block, name, new_multi_head_lora)
                        list_lora_layers.append(new_multi_head_lora)

    if lora_dict['encoder'] == 'vision' or lora_dict['encoder'] == 'both':
        indices = INDEX_POSITIONS_VISION[lora_dict['model']][lora_dict['position']]

        if clip_model.clip_type == 'open_clip':
                ...
                if i in indices and i in target_layers:
                    ...
                    for name, submodule in block.named_children():
                        if isinstance(submodule, nn.MultiheadAttention):
                            ...
                            setattr(block, name, new_multi_head_lora)
                            list_lora_layers.append(new_multi_head_lora)
        elif clip_model.clip_type == 'conch':
            import timm
            ...
            for i, block in enumerate(vision_encoder.blocks):
                if i in indices and i in target_layers:
                    print(f"Applying Lora on Attention Block {i}")
                    for name, submodule in block.named_children():
                        # if isinstance(submodule, timm.models.vision_transformer.Attention):
                        #     new_multi_head_lora = TimmAttentionLoRA(
                        #         submodule, enable_lora=lora_dict['params'], r=lora_dict['r'], lora_alpha=lora_dict['alpha'],
                        #         dropout_rate=lora_dict['dropout_rate'])

                        #     setattr(block, name, new_multi_head_lora)
                        #     list_lora_layers.append(new_multi_head_lora)
                        if isinstance(submodule, TorchMHAWrapper):
                            new_multi_head_lora = PlainMultiheadAttentionLoRA(
                                submodule.mha, enable_lora=lora_dict['params'], r=lora_dict['r'], lora_alpha=lora_dict['alpha'],
                                dropout_rate=lora_dict['dropout_rate'])
                            setattr(submodule, 'mha', new_multi_head_lora)
                            list_lora_layers.append(new_multi_head_lora)
        else:
            raise NotImplementedError(f"Unknown CLIP type {clip_model.clip_type}")
        ###
...
def load_lora(args, list_lora_layers):
    # to manage names like ViT-B/16
    backbone = args.model.replace('/', '').replace('-', '').lower()
